...
export default function App() {
  ...
  // 🆕 Leer query params del link de WhatsApp Bot
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const lat = parseFloat(params.get("to_lat"));
    const lng = parseFloat(params.get("to_lng"));
    const name = params.get("to_name");
    const perfilParam = params.get("perfil");
    if (!isNaN(lat) && !isNaN(lng)) {
      setPreloadedDest({ lat, lng, name: name || "Destino", label: name || "Destino" });
      setTab("route");
    }
    if (perfilParam && PROFILES.some(p => p.id === perfilParam)) {
      setProfile(perfilParam); // si vino perfil, saltamos pantalla de bienvenida
    }
  }, []);
