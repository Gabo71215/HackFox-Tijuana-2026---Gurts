...
function RouteView({ profile, reports, preloadedDest }) {
  ...
  // 🆕 Si vino destino desde el link del bot de WhatsApp, precargarlo
  useEffect(() => {
    if (preloadedDest && destRef.current) {
      setDest({ lat: preloadedDest.lat, lng: preloadedDest.lng });
      destRef.current.value = preloadedDest.name || "Destino del bot";
      // Auto-llenar origen con GPS si está vacío
                    ...
                    {[
                      {
                        ...
                        emoji: "🚌",
                        href: origin && dest
                          ? `https://moovit.com/?from=${origin.lat},${origin.lng}&to=${dest.lat},${dest.lng}`
                          : `https://moovit.com/?to=${dest.lat},${dest.lng}`,
                      },
                      {
                        name: "Google Maps",
                        emoji: "🗺️",
                      ...
                      <a key={app.name} href={app.href} target="_blank" rel="noopener noreferrer" className="tap" style={{
...
`Eres el asistente de accesibilidad de Tijuana Accesible (app del Ayuntamiento de Tijuana). Tienes acceso a la ubicación del usuario y a los reportes ciudadanos de barreras. Responde en español, breve y útil. Si el usuario pregunta cómo llegar a un lugar, sugiere la pestaña "Ruta" de la app — ahí calculamos la ruta accesible certificada con pendiente real y barreras evitadas, y también ofrecemos abrir directo en Moovit (transporte público), Google Maps o Apple Maps. Si pregunta dónde reportar algo, sugiere las pestañas "Foto" o "Voz". Da datos concretos con distancia cuando puedas. La troncal del SITT es la única línea de transporte accesible de Tijuana (rampas, piso táctil, abordaje a nivel). Las calafias NO son accesibles.
${context}
